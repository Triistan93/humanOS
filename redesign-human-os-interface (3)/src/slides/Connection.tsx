...
import { Check, Lightbulb, Hand, MousePointer2, BookOpen } from "lucide-react";
...
const ITEMS = [
  { icon: Check, text: "Permissão para admitir um erro precocemente sem medo de punição." },
  { icon: Lightbulb, text: "Espaço seguro para testar ideias arriscadas ou contra-intuitivas." },
  { icon: Hand, text: "Falar sobre vulnerabilidades do processo sem ser taxado de 'incompetente'." },
];
      ...
      <div className="grid w-full max-w-[1500px] items-center gap-10 lg:grid-cols-12 lg:gap-16">
        ...
        <div className="space-y-6 lg:col-span-6">
          ...
          <Reveal active={active} delay={0.18}>
            <div className="inline-flex items-center gap-2 rounded-lg border border-poison/30 bg-poison/10 px-3 py-1.5 font-mono text-[10px] text-poison uppercase tracking-wider">
              <BookOpen size={13} />
              <span>Dra. Amy Edmondson (Harvard) + Project Aristotle (Google, Estudo em 180 Equipes)</span>
            </div>
          </Reveal>
          ...
          <Reveal active={active} delay={0.24} className="space-y-4">
            <div className="border-l-4 border-poison pl-5">
              <h3 className="font-mono text-sm font-bold uppercase tracking-wider text-white md:text-lg">
                A Variável #1 do Google Aristotle
              </h3>
              <p className="mt-2 max-w-xl text-sm font-light leading-relaxed text-slate-300 md:text-lg">
                A pesquisa de Harvard comprovou: a segurança psicológica não é ser bonzinho, é a crença compartilhada de que a equipe é segura para assumir riscos interpessoais.
              </p>
            </div>
                ...
                return (
                  <Reveal key={i} active={active} delay={0.3 + i * 0.1}>
                    <div className="flex items-center gap-4">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-poison/30 bg-poison/10 text-poison shadow-[0_0_20px_-6px_rgba(57,255,20,0.5)]">
                        <Icon size={16} strokeWidth={2} />
                      </div>
                      <span className="text-sm font-light text-slate-200 md:text-base">{it.text}</span>
                    </div>
                  </Reveal>
                );
              })}
        ...
        {/* Right Column — Trust Canvas */}
        <Reveal active={active} delay={0.25} className="lg:col-span-6">
          <div
            className="glass corner-frame relative h-[46vh] overflow-hidden rounded-2xl md:h-[60vh]"
            style={{ borderColor: "rgba(57,255,20,0.25)", boxShadow: "0 0 50px -16px rgba(57,255,20,0.25)" }}
          >
            <TrustCanvas />
            <div className="pointer-events-none absolute left-5 top-5 flex items-center gap-3 font-mono text-[9px] uppercase tracking-[0.3em] text-poison/80 md:text-[10px]">
              <MousePointer2 size={12} />
              Rede de Confiança // Edmondson Matrix
            </div>
            <div className="pointer-events-none absolute bottom-5 right-5 font-mono text-[9px] uppercase tracking-[0.3em] text-slate-500 md:text-[10px]">
              Harvard & Google Dataset // 34 nós ativos
            </div>
          </div>
        </Reveal>
      </div>
